...
use crate::{
    menfugu_transition_digest, MenfuguCompiledQsm, MenfuguLoweredTransition, MenfuguPublicInput,
    MenfuguPublicOutput, MenfuguPublicState, MENFUGU_OUT_OF_ORDER_PUBLIC_STEP,
    MENFUGU_PUBLIC_FAULT, MENFUGU_PUBLIC_REJECT, MENFUGU_UNKNOWN_PUBLIC_INPUT,
    MENFUGU_UNKNOWN_PUBLIC_SERVICE,
};

pub const MENFUGU_DIFFERENTIAL_VERSION: &str = "noticer-menfugu-differential/v1";
const SMALL_STEP_ADAPTER_VERSION: &str = "noticer-menfugu-small-step-adapter/v1";
const SOURCE_REFERENCE_VERSION: &str = "noticer-menfugu-source-reference/v1";
const HARDWARE_STATUS: &str = "NOT_VERIFIED";
const SEQUENCE_MAGIC: &[u8; 8] = b"MENSEQ01";
const SEQUENCE_DIGEST_DOMAIN: &[u8] = b"noticer-menfugu-public-sequence-v1";
const REFERENCE_EXECUTABLE_DOMAIN: &[u8] = b"noticer-menfugu-source-reference-code-v1";

#[derive(Clone, Debug, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct MenfuguEngineDigests {
    small_step_sha256: String,
    wasmi_sha256: String,
    wasmtime_sha256: String,
}

impl MenfuguEngineDigests {
    pub fn new(
        small_step_sha256: impl Into<String>,
        wasmi_sha256: impl Into<String>,
        wasmtime_sha256: impl Into<String>,
    ) -> Result<Self, MenfuguDifferentialError> {
        let digests = Self {
        ...
        for (engine, digest) in [
            ...
            if !is_sha256(digest) {
                return Err(MenfuguDifferentialError::InvalidExecutableDigest {
...
pub enum MenfuguDifferentialVerdict {
    Match,
    Counterexample,
    Unresolved,
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq, Serialize)]
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum MenfuguDifferentialEvidenceOrigin {
    ExecutedSoftware,
    InjectedTestFixture,
}
...
#[serde(deny_unknown_fields)]
pub struct MenfuguExpectedTransition {
    pub source_state: u8,
    pub public_input: u8,
    pub target_state: u8,
...
#[derive(Clone, Debug, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct MenfuguSourceRefinement {
...
pub struct MenfuguPublicSequence {
...
impl MenfuguPublicSequence {
    pub fn new(
        ...
        if commands.is_empty() || commands.len() > max_commands || max_commands == 0 {
            return Err(MenfuguDifferentialError::CommandCount);
        }
        if limits.fuel == 0
            ...
            || limits.timeout_ms == 0
        {
            return Err(MenfuguDifferentialError::InvalidLimits);
        }
...
#[derive(Clone, Debug, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct MenfuguDifferentialArtifact {
    ...
    pub evidence_origin: MenfuguDifferentialEvidenceOrigin,
    pub injection_label: Option<String>,
    pub engine_digests: MenfuguEngineDigests,
    pub source_transitions: Vec<MenfuguExpectedTransition>,
    pub verdict: MenfuguDifferentialVerdict,
    pub source_reference: Option<EngineRunArtifact>,
    pub source_refinement: MenfuguSourceRefinement,
    pub oracle: DifferentialOracleArtifact,
}

impl MenfuguDifferentialArtifact {
    pub fn validate(&self) -> Result<(), MenfuguDifferentialError> {
        if self.schema_version != MENFUGU_DIFFERENTIAL_VERSION
            || self.evaluator_version != MENFUGU_DIFFERENTIAL_VERSION
        ...
        {
            return Err(MenfuguDifferentialError::ArtifactContract);
        }
        self.oracle
            .validate()
            .map_err(|error| MenfuguDifferentialError::Oracle(error.to_string()))?;
    ...
    pub fn canonical_json(&self) -> Result<Vec<u8>, MenfuguDifferentialError> {
        self.validate()?;
        serde_json::to_vec(self)
            .map_err(|error| MenfuguDifferentialError::Serialization(error.to_string()))
    }

    pub fn artifact_sha256(&self) -> Result<String, MenfuguDifferentialError> {
        Ok(sha256_hex(&self.canonical_json()?))
    }
}

pub fn evaluate_menfugu_differential(
    compiled: &MenfuguCompiledQsm,
    sequence: &MenfuguPublicSequence,
    engine_digests: &MenfuguEngineDigests,
) -> Result<MenfuguDifferentialArtifact, MenfuguDifferentialError> {
    evaluate_menfugu_differential_with_host_tape(
        compiled,
        sequence,
        sequence.host_tape(),
        engine_digests,
...
pub fn evaluate_menfugu_differential_with_host_tape(
    compiled: &MenfuguCompiledQsm,
    sequence: &MenfuguPublicSequence,
    host_tape: &PublicHostTape,
    engine_digests: &MenfuguEngineDigests,
    ...
    let artifact = MenfuguDifferentialArtifact {
        schema_version: MENFUGU_DIFFERENTIAL_VERSION.to_owned(),
        evaluator_version: MENFUGU_DIFFERENTIAL_VERSION.to_owned(),
        ...
        evidence_origin: MenfuguDifferentialEvidenceOrigin::ExecutedSoftware,
...
pub fn build_menfugu_injected_fixture_artifact(
    base: &MenfuguDifferentialArtifact,
    ...
    if !valid_injection_label(&injection_label) {
        return Err(MenfuguDifferentialError::InjectionLabel);
    }
    let source_refinement = source_refinement(
    ...
    artifact.evidence_origin = MenfuguDifferentialEvidenceOrigin::InjectedTestFixture;
...
fn expected_source_transitions(compiled: &MenfuguCompiledQsm) -> Vec<MenfuguExpectedTransition> {
    compiled
        .transitions()
        .iter()
        .map(|transition| MenfuguExpectedTransition {
...
fn source_transitions_are_canonical(
    transitions: &[MenfuguExpectedTransition],
    expected_digest: &str,
) -> bool {
    if transitions.len() != MenfuguPublicState::ALL.len() * MenfuguPublicInput::ALL.len() {
        return false;
    }
    ...
    for transition in transitions {
        ...
        lowered.push(MenfuguLoweredTransition {
            source_state,
            public_input,
            target_state,
            public_output,
    ...
    let canonical = lowered.iter().enumerate().all(|(index, transition)| {
        let state_index = index / MenfuguPublicInput::ALL.len();
        let input_index = index % MenfuguPublicInput::ALL.len();
        transition.source_state == MenfuguPublicState::ALL[state_index]
            && transition.public_input == MenfuguPublicInput::ALL[input_index]
    });
    canonical && hex(menfugu_transition_digest(&lowered).as_bytes()) == expected_digest
}

fn evidence_origin_is_valid(
    origin: MenfuguDifferentialEvidenceOrigin,
    injection_label: Option<&str>,
) -> bool {
    match (origin, injection_label) {
        (MenfuguDifferentialEvidenceOrigin::ExecutedSoftware, None) => true,
        (MenfuguDifferentialEvidenceOrigin::InjectedTestFixture, Some(label)) => {
...
fn validate_commands(commands: &[ContextCommand]) -> Result<(), MenfuguDifferentialError> {
    let mut stopped = false;
    for command in commands {
        if stopped {
            return Err(MenfuguDifferentialError::CommandAfterStop);
        }
        if command.kind != command.family.command_kind() || command.payload_tag != 0 {
            return Err(MenfuguDifferentialError::NonCanonicalCommand);
        }
        match command.kind {
            CommandKind::PublicCall | CommandKind::PublicFault => {
                let input = decode_input(command.fault)
                    .ok_or(MenfuguDifferentialError::NonCanonicalCommand)?;
...
const fn input_family_matches(input: MenfuguPublicInput, family: ContextFamily) -> bool {
    match input {
        MenfuguPublicInput::AuthorizedAction
        | MenfuguPublicInput::Cover
        | MenfuguPublicInput::PumpStopped
        | MenfuguPublicInput::CooldownElapsed
        | MenfuguPublicInput::Reset
        | MenfuguPublicInput::Handoff => matches!(family, ContextFamily::Tick),
        MenfuguPublicInput::ReplayRejected => matches!(family, ContextFamily::CrossServiceReplay),
        MenfuguPublicInput::ExpiredRejected | MenfuguPublicInput::Deadline => {
            matches!(family, ContextFamily::Deadline)
        }
        ...
        MenfuguPublicInput::Fault => matches!(family, ContextFamily::FaultTimeout),
    }
}

fn decode_state(code: u8) -> Option<MenfuguPublicState> {
    match code {
        0 => Some(MenfuguPublicState::Ready),
        1 => Some(MenfuguPublicState::Executing),
        2 => Some(MenfuguPublicState::Cooldown),
        3 => Some(MenfuguPublicState::FailClosed),
        _ => None,
    }
}

fn decode_input(code: u8) -> Option<MenfuguPublicInput> {
    match code {
        0 => Some(MenfuguPublicInput::AuthorizedAction),
        1 => Some(MenfuguPublicInput::Cover),
        2 => Some(MenfuguPublicInput::ReplayRejected),
        3 => Some(MenfuguPublicInput::ExpiredRejected),
        4 => Some(MenfuguPublicInput::WrongServiceRejected),
        5 => Some(MenfuguPublicInput::WrongPolicyRejected),
        6 => Some(MenfuguPublicInput::WrongKeyRejected),
        7 => Some(MenfuguPublicInput::DuplicateTransport),
        8 => Some(MenfuguPublicInput::PumpStopped),
        9 => Some(MenfuguPublicInput::CooldownElapsed),
        10 => Some(MenfuguPublicInput::Reset),
        11 => Some(MenfuguPublicInput::Handoff),
        12 => Some(MenfuguPublicInput::Deadline),
        13 => Some(MenfuguPublicInput::Fault),
        _ => None,
    }
}

fn decode_output(code: u8) -> Option<MenfuguPublicOutput> {
    match code {
        0 => Some(MenfuguPublicOutput::NoOp),
        1 => Some(MenfuguPublicOutput::Cover),
        2 => Some(MenfuguPublicOutput::ExecuteOnce),
        3 => Some(MenfuguPublicOutput::Reject),
        4 => Some(MenfuguPublicOutput::Stop),
        5 => Some(MenfuguPublicOutput::StopAndReset),
        6 => Some(MenfuguPublicOutput::StopAndHandoff),
        7 => Some(MenfuguPublicOutput::FailClosed),
        _ => None,
    }
}
...
fn plan_tick(
    ...
    if command.public_slot != cursor.wrapping_add(1) {
        return Ok(TickPlan {
            calls: vec![PlannedHostCall::Failure {
                code: MENFUGU_OUT_OF_ORDER_PUBLIC_STEP,
            }],
            next_state: state,
            next_cursor: cursor,
        });
    ...
    let Some(input) = decode_input(command.fault) else {
        return Ok(TickPlan {
            calls: vec![PlannedHostCall::Failure {
                code: MENFUGU_UNKNOWN_PUBLIC_INPUT,
            }],
            next_state: state,
            next_cursor: cursor,
        });
    ...
    let transition = compiled
        ...
        .ok_or(MenfuguDifferentialError::TransitionCoverage)?;
    let mut calls = vec![PlannedHostCall::Frame {
        label: command.service_alias,
        slot: command.public_slot,
    }];
    match transition.public_output {
        MenfuguPublicOutput::NoOp
        | MenfuguPublicOutput::Cover
        | MenfuguPublicOutput::Stop
        | MenfuguPublicOutput::StopAndReset
        | MenfuguPublicOutput::StopAndHandoff => {}
        MenfuguPublicOutput::ExecuteOnce => calls.push(PlannedHostCall::Action {
            action: compiled.action_code(),
            slot: u32::try_from(command.public_slot)
                .map_err(|_| MenfuguDifferentialError::Arithmetic)?,
        }),
        MenfuguPublicOutput::Reject => calls.push(PlannedHostCall::Failure {
            code: MENFUGU_PUBLIC_REJECT,
        }),
        MenfuguPublicOutput::FailClosed => calls.push(PlannedHostCall::Failure {
            code: MENFUGU_PUBLIC_FAULT,
        }),
    }
    Ok(TickPlan {
        calls,
...
) -> Result<PublicHostTape, MenfuguDifferentialError> {
    let mut directives = Vec::new();
    let mut state = MenfuguPublicState::Ready;
    ...
    for command in commands {
        match command.kind {
            ...
            CommandKind::PublicReset => {
                state = MenfuguPublicState::Ready;
                cursor = u64::MAX;
            }
            CommandKind::PublicHandoff => state = MenfuguPublicState::Ready,
            CommandKind::Stop => {}
        }
    }
...
fn evaluate_source_reference(
    compiled: &MenfuguCompiledQsm,
    sequence: &MenfuguPublicSequence,
) -> Result<SourceReferenceVerdict, MenfuguDifferentialError> {
    ...
    let run = EngineRunArtifact::new(input, trace, termination, EngineRunVerdict::Executed)
        .map_err(|error| MenfuguDifferentialError::EngineContract(error.to_string()))?;
...
fn expected_trace(
    compiled: &MenfuguCompiledQsm,
    sequence: &MenfuguPublicSequence,
) -> Result<(Vec<ObservableEvent>, ExecutionTermination), MenfuguDifferentialError> {
...
fn append_public_state_probe(trace: &mut Vec<ObservableEvent>, state: MenfuguPublicState) {
...
fn sequence_digest(
    ...
    host_tape: &PublicHostTape,
    limits: ExecutionLimits,
) -> Result<Digest, MenfuguDifferentialError> {
...
fn source_reference_identity(compiled: &MenfuguCompiledQsm) -> EngineIdentity {
    let executable = artifact_digest(
        REFERENCE_EXECUTABLE_DOMAIN,
        SOURCE_REFERENCE_VERSION.as_bytes(),
    );
    EngineIdentity {
        name: "noticer-menfugu-source-reference".to_owned(),
...
fn validate_host_tape_shape(
...
) -> Result<(), MenfuguDifferentialError> {
    ...
    {
        Err(MenfuguDifferentialError::HostTapeShape)
    } else {
        Ok(())
    }
}
...
fn project_source_reference(
    ...
    host_tape: &PublicHostTape,
) -> Result<EngineRunArtifact, MenfuguDifferentialError> {
    ...
    for event in &base.trace {
        ...
        let directive = host_tape
            .directives()
            .get(directive_index)
            .ok_or(MenfuguDifferentialError::HostTapeShape)?;
        if directive.import() != import {
            return Err(MenfuguDifferentialError::HostTapeShape);
        }
        directive_index = directive_index
            .checked_add(1)
            .ok_or(MenfuguDifferentialError::ArtifactContract)?;
...
fn small_step_tape(host_tape: &PublicHostTape) -> Result<PublicHostTape, MenfuguDifferentialError> {
    let directives = host_tape
        ...
        .map(|directive| {
            let import = directive
                .import()
                .strip_prefix("qseal.")
                .ok_or(MenfuguDifferentialError::HostImportContract)?;
            Ok(HostDirective::new(import, directive.outcome()))
        })
        .collect::<Result<Vec<_>, MenfuguDifferentialError>>()?;
    Ok(PublicHostTape::new(directives))
}
...
fn append_lifecycle_event(
    ...
    match kind {
        CommandKind::PublicReset => match values {
            ...
            _ => return Err(MenfuguDifferentialError::ReturnShape("reset")),
        },
        CommandKind::PublicHandoff => match values {
            [Value::I64(value)] => trace.push(ObservableEvent::Handoff { value: *value }),
            _ => return Err(MenfuguDifferentialError::ReturnShape("handoff")),
        },
        CommandKind::PublicCall | CommandKind::PublicFault => {}
...
fn source_refinement(
...
) -> Result<MenfuguSourceRefinement, MenfuguDifferentialError> {
    let Some(source) = source else {
        return Ok(MenfuguSourceRefinement {
    ...
    if !shared_input_matches(&source.input, &small_step.input) {
        return Err(MenfuguDifferentialError::SourceInputMismatch);
    }
    ...
    if let Some(first_difference) = first_difference(source, small_step) {
        ...
        Ok(MenfuguSourceRefinement {
            verdict: MenfuguDifferentialVerdict::Match,
            first_difference: None,
            unresolved_reason: None,
        })
    }
...
const fn aggregate_verdict(
    source: MenfuguDifferentialVerdict,
    oracle: DifferentialVerdict,
) -> MenfuguDifferentialVerdict {
    match (source, oracle) {
        (MenfuguDifferentialVerdict::Unresolved, _) | (_, DifferentialVerdict::Unresolved) => {
            MenfuguDifferentialVerdict::Unresolved
        }
        (MenfuguDifferentialVerdict::Counterexample, _)
        | (_, DifferentialVerdict::Counterexample) => MenfuguDifferentialVerdict::Counterexample,
        (MenfuguDifferentialVerdict::Match, DifferentialVerdict::Match) => {
...
fn external_error(engine: &'static str, error: impl std::fmt::Display) -> MenfuguDifferentialError {
    MenfuguDifferentialError::ExternalEngine {
        engine,
...
#[derive(Debug, Error)]
pub enum MenfuguDifferentialError {
    ...
    #[error("MENFUGU differential artifact serialization failed: {0}")]
    Serialization(String),
    #[error("MENFUGU differential artifact violated its contract")]
    ArtifactContract,
    ...
    HostTapeShape,
    #[error("MENFUGU public sequence command count is empty or exceeds the bound")]
    CommandCount,
    #[error("MENFUGU public sequence resource limits must be nonzero")]
    InvalidLimits,
    #[error("MENFUGU public sequence contains a non-canonical command")]
    NonCanonicalCommand,
    #[error("MENFUGU public sequence contains a command after Stop")]
    CommandAfterStop,
    #[error("MENFUGU compiled transition table is incomplete")]
    TransitionCoverage,
    #[error("MENFUGU differential arithmetic overflow")]
    Arithmetic,
