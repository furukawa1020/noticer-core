...
use crate::{Atv2CompiledQsm, Atv2PublicSequence};

pub const ATV2_ADVERSARIAL_MATRIX_VERSION: &str = "noticer-atv2-adversarial-matrix/v1";
const FORMAT_VERSION: u16 = 1;
const MAGIC: &[u8; 8] = b"ATV2MTX1";
const CASE_ID_DOMAIN: &[u8] = b"noticer-atv2-adversarial-case-id/v1";
const MATRIX_DIGEST_DOMAIN: &[u8] = b"noticer-atv2-adversarial-matrix-digest/v1";

#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct Atv2MatrixSeed([u8; 32]);
...
#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct Atv2MatrixDigest([u8; 32]);

impl Atv2MatrixDigest {
    #[must_use]
    pub const fn as_bytes(&self) -> &[u8; 32] {
        &self.0
    }
...
impl Atv2ScenarioAxis {
    fn from_code(code: u8) -> Result<Self, Atv2MatrixError> {
        match code {
            0 => Ok(Self::Normal),
            ...
            _ => Err(Atv2MatrixError::UnknownAxis {
                axis: "scenario",
                code,
            }),
        }
...
impl Atv2HostAxis {
    fn from_code(code: u8) -> Result<Self, Atv2MatrixError> {
        match code {
            ...
            3 => Ok(Self::Reconnect),
            4 => Ok(Self::Loss),
            _ => Err(Atv2MatrixError::UnknownAxis { axis: "host", code }),
        }
    }
}
...
pub struct Atv2MatrixLimits {
    pub max_artifact_bytes: usize,
    pub max_cases: usize,
    pub max_commands_per_case: usize,
}

impl Atv2MatrixLimits {
    #[must_use]
    ...
    fn validate(self) -> Result<(), Atv2MatrixError> {
        if self.max_artifact_bytes == 0 || self.max_cases == 0 || self.max_commands_per_case == 0 {
            Err(Atv2MatrixError::InvalidDecodeLimits)
        } else {
            Ok(())
        }
    }
...
impl Default for Atv2MatrixLimits {
    fn default() -> Self {
        Self::frozen_v1()
    }
}
...
pub struct Atv2AdversarialCaseSpec {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct Atv2AdversarialCase {
    scenario: Atv2ScenarioAxis,
    host: Atv2HostAxis,
    resource: Atv2ResourceAxis,
...
impl Atv2AdversarialCase {
    ...
    #[must_use]
    pub const fn case_id(&self) -> Atv2CaseId {
        self.case_id
    }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct Atv2AdversarialMatrix {
    seed: Atv2MatrixSeed,
    source_digest: [u8; 32],
    module_digest: [u8; 32],
    capsule_digest: [u8; 32],
    abi_digest: [u8; 32],
    cases: Box<[Atv2AdversarialCase]>,
    matrix_digest: Atv2MatrixDigest,
}

impl Atv2AdversarialMatrix {
    pub fn new(
        compiled: &Atv2CompiledQsm,
        seed: Atv2MatrixSeed,
        specs: Vec<Atv2AdversarialCaseSpec>,
        limits: Atv2MatrixLimits,
    ) -> Result<Self, Atv2MatrixError> {
        limits.validate()?;
        if specs.is_empty() || specs.len() > limits.max_cases {
            return Err(Atv2MatrixError::CaseCount);
        }
        ...
        for (index, spec) in specs.into_iter().enumerate() {
            ...
            .map_err(|error| Atv2MatrixError::PublicSequence {
                case_index: index,
                detail: error.to_string(),
            })?;
            let mut case = Atv2AdversarialCase {
        ...
        cases.sort_by_key(Atv2AdversarialCase::case_id);
        ...
        let mut matrix = Self {
            ...
            matrix_digest: Atv2MatrixDigest([0; 32]),
        };
        matrix.matrix_digest = matrix.compute_matrix_digest()?;
        if matrix.canonical_bytes()?.len() > limits.max_artifact_bytes {
            return Err(Atv2MatrixError::ArtifactSize);
        }
        Ok(matrix)
    }

    pub fn from_bytes(bytes: &[u8], limits: Atv2MatrixLimits) -> Result<Self, Atv2MatrixError> {
        limits.validate()?;
        ...
        if reader.read_array::<8>()? != *MAGIC {
            return Err(Atv2MatrixError::Magic);
        }
        let version = reader.read_u16()?;
        if version != FORMAT_VERSION {
            return Err(Atv2MatrixError::Version(version));
        }
        let seed = Atv2MatrixSeed(reader.read_array()?);
        ...
        for _ in 0..case_count {
            let scenario = Atv2ScenarioAxis::from_code(reader.read_u8()?)?;
            let host = Atv2HostAxis::from_code(reader.read_u8()?)?;
            let resource = Atv2ResourceAxis::from_code(reader.read_u8()?)?;
            ...
            if command_count == 0 || command_count > limits.max_commands_per_case {
                return Err(Atv2MatrixError::CommandCount);
            }
            ...
            let case_id = Atv2CaseId(reader.read_array()?);
            let case = Atv2AdversarialCase {
                scenario,
                host,
                resource,
            ...
            if expected != case.case_id {
                return Err(Atv2MatrixError::CaseIdMismatch);
            }
            cases.push(case);
        }
        validate_case_set(&cases)?;
        let matrix_digest = Atv2MatrixDigest(reader.read_array()?);
        reader.finish()?;
        let matrix = Self {
            seed,
            source_digest,
        ...
        if matrix.compute_matrix_digest()? != matrix.matrix_digest {
            return Err(Atv2MatrixError::MatrixDigestMismatch);
        }
        Ok(matrix)
    }
    ...
    pub fn validate_against(
        ...
        {
            return Err(Atv2MatrixError::CompiledBindingMismatch);
        }
        let canonical = self.canonical_bytes()?;
        if canonical.len() > limits.max_artifact_bytes
            || Self::from_bytes(&canonical, limits)? != *self
        {
            return Err(Atv2MatrixError::ArtifactContract);
        }
        for (index, case) in self.cases.iter().enumerate() {
            ...
            if sequence.digest().as_bytes() != &case.sequence_digest {
                return Err(Atv2MatrixError::SequenceDigestMismatch { case_index: index });
            }
        }
        Ok(())
    }
    ...
    #[must_use]
    pub const fn seed(&self) -> Atv2MatrixSeed {
        self.seed
    }

    #[must_use]
    pub fn cases(&self) -> &[Atv2AdversarialCase] {
        &self.cases
    }

    #[must_use]
    pub const fn matrix_digest(&self) -> Atv2MatrixDigest {
        self.matrix_digest
    }

    pub fn canonical_bytes(&self) -> Result<Vec<u8>, Atv2MatrixError> {
        let mut bytes = self.encode_without_matrix_digest()?;
        bytes.extend_from_slice(self.matrix_digest.as_bytes());
        Ok(bytes)
    }

    fn compute_matrix_digest(&self) -> Result<Atv2MatrixDigest, Atv2MatrixError> {
        Ok(Atv2MatrixDigest(domain_hash(
            MATRIX_DIGEST_DOMAIN,
            &self.encode_without_matrix_digest()?,
        )))
    }

    fn encode_without_matrix_digest(&self) -> Result<Vec<u8>, Atv2MatrixError> {
...
fn compute_case_id(
    ...
    capsule_digest: &[u8; 32],
    abi_digest: &[u8; 32],
    case: &Atv2AdversarialCase,
) -> Result<Atv2CaseId, Atv2MatrixError> {
    let mut bytes = Vec::new();
    ...
    put_case(&mut bytes, case)?;
    Ok(Atv2CaseId(domain_hash(CASE_ID_DOMAIN, &bytes)))
}

fn put_case(bytes: &mut Vec<u8>, case: &Atv2AdversarialCase) -> Result<(), Atv2MatrixError> {
...
fn validate_case_set(cases: &[Atv2AdversarialCase]) -> Result<(), Atv2MatrixError> {
    let mut tuples = BTreeSet::new();
    let mut previous = None;
    for case in cases {
        if let Some(previous) = previous {
            if case.case_id == previous {
                return Err(Atv2MatrixError::DuplicateCaseId);
            }
            if case.case_id < previous {
                return Err(Atv2MatrixError::NonCanonicalOrder);
            }
        }
        previous = Some(case.case_id);
        if !tuples.insert((case.scenario, case.host, case.resource)) {
            return Err(Atv2MatrixError::DuplicateAxisTuple);
        }
    }
    Ok(())
}
...
fn validate_commands(
    ...
    if limits.fuel == 0
        ...
        || limits.timeout_ms == 0
    {
        return Err(Atv2MatrixError::ExecutionLimits);
    }
    let mut stopped = false;
    for command in commands {
        if stopped || command.kind != command.family.command_kind() || command.payload_tag != 0 {
            return Err(Atv2MatrixError::CommandContract);
        }
        match command.family {
            ...
            ContextFamily::Malformed => return Err(Atv2MatrixError::CommandContract),
        }
...
fn validate_fault(command: &ContextCommand, expected: u8) -> Result<(), Atv2MatrixError> {
...
fn read_command(reader: &mut Reader<'_>) -> Result<ContextCommand, Atv2MatrixError> {
    ...
    let family = match family_code {
        ...
        code => return Err(Atv2MatrixError::UnknownContextFamily(code)),
    };
    ...
    let kind = match kind_code {
        ...
        4 => CommandKind::Stop,
        code => return Err(Atv2MatrixError::UnknownCommandKind(code)),
    };
    Ok(ContextCommand {
        family,
        kind,
...
fn put_len(bytes: &mut Vec<u8>, len: usize) -> Result<(), Atv2MatrixError> {
    let len = u32::try_from(len).map_err(|_| Atv2MatrixError::Arithmetic)?;
...
impl<'a> Reader<'a> {
    ...
    fn read_array<const N: usize>(&mut self) -> Result<[u8; N], Atv2MatrixError> {
        let slice = self.read_exact(N)?;
        slice.try_into().map_err(|_| Atv2MatrixError::Truncated)
    }

    fn read_u8(&mut self) -> Result<u8, Atv2MatrixError> {
        Ok(self.read_array::<1>()?[0])
    }

    fn read_u16(&mut self) -> Result<u16, Atv2MatrixError> {
        Ok(u16::from_le_bytes(self.read_array()?))
    }

    fn read_u32(&mut self) -> Result<u32, Atv2MatrixError> {
        Ok(u32::from_le_bytes(self.read_array()?))
    }

    fn read_u64(&mut self) -> Result<u64, Atv2MatrixError> {
        Ok(u64::from_le_bytes(self.read_array()?))
    }

    fn read_exact(&mut self, len: usize) -> Result<&'a [u8], Atv2MatrixError> {
        let end = self
            .cursor
            .checked_add(len)
            .ok_or(Atv2MatrixError::Arithmetic)?;
        let slice = self
            .bytes
            .get(self.cursor..end)
            .ok_or(Atv2MatrixError::Truncated)?;
        self.cursor = end;
        Ok(slice)
    }

    fn finish(self) -> Result<(), Atv2MatrixError> {
...
#[derive(Debug, Error)]
pub enum Atv2MatrixError {
    #[error("ATV2 matrix decode limits must be nonzero")]
    InvalidDecodeLimits,
    #[error("ATV2 matrix case count is empty or exceeds the bound")]
    CaseCount,
    #[error("ATV2 matrix command count is empty or exceeds the bound")]
    CommandCount,
    #[error("ATV2 matrix execution limits must be nonzero")]
    ExecutionLimits,
    #[error("ATV2 matrix command violates the public command contract")]
    CommandContract,
    #[error("ATV2 matrix public sequence failed for case {case_index}: {detail}")]
    PublicSequence { case_index: usize, detail: String },
    #[error("ATV2 matrix artifact exceeds the byte bound")]
    ArtifactSize,
    #[error("ATV2 matrix arithmetic overflow")]
    Arithmetic,
    #[error("ATV2 matrix magic is invalid")]
    Magic,
    #[error("unsupported ATV2 matrix format version {0}")]
    Version(u16),
    #[error("ATV2 matrix input is truncated")]
    Truncated,
    #[error("unknown ATV2 matrix {axis} axis code {code}")]
    ...
    #[error("ATV2 matrix case ID does not match its payload")]
    CaseIdMismatch,
    #[error("ATV2 matrix case IDs are not in canonical order")]
    NonCanonicalOrder,
    #[error("ATV2 matrix contains a duplicate case ID")]
    DuplicateCaseId,
    #[error("ATV2 matrix contains a duplicate axis tuple")]
    DuplicateAxisTuple,
    #[error("ATV2 matrix digest does not match its canonical payload")]
    MatrixDigestMismatch,
    #[error("ATV2 matrix contains trailing bytes")]
    TrailingBytes,
    #[error("ATV2 matrix compiled artifact bindings do not match")]
    CompiledBindingMismatch,
    #[error("ATV2 matrix sequence digest differs for case {case_index}")]
    SequenceDigestMismatch { case_index: usize },
    #[error("ATV2 matrix object violated its canonical artifact contract")]
    ArtifactContract,
}
