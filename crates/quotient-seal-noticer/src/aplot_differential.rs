...
use crate::{
    evaluate_aplot_source_reference, AplotCompiledQsm, AplotPublicEventKind, AplotPublicSequence,
    AplotReferenceVerdict, APLOT_PUBLIC_DEADLINE, APLOT_PUBLIC_LOSS, APLOT_PUBLIC_RECONNECT,
};

pub const APLOT_DIFFERENTIAL_VERSION: &str = "noticer-aplot-differential/v1";
const SMALL_STEP_ADAPTER_VERSION: &str = "noticer-aplot-small-step-adapter/v1";
const HARDWARE_STATUS: &str = "NOT_VERIFIED";

#[derive(Clone, Debug, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct AplotEngineDigests {
    small_step_sha256: String,
    wasmi_sha256: String,
    wasmtime_sha256: String,
}

impl AplotEngineDigests {
    pub fn new(
        small_step_sha256: impl Into<String>,
        wasmi_sha256: impl Into<String>,
        wasmtime_sha256: impl Into<String>,
    ) -> Result<Self, AplotDifferentialError> {
        let digests = Self {
        ...
        for (engine, digest) in [
            ...
            if !is_sha256(digest) {
                return Err(AplotDifferentialError::InvalidExecutableDigest {
...
pub enum AplotDifferentialVerdict {
    Match,
    Counterexample,
    Unresolved,
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq, Serialize)]
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum AplotExpectedEventKind {
    FragmentAttempt,
    Reconnect,
    Deadline,
}
...
pub struct AplotExpectedEvent {
    ...
    pub sequence: u32,
    pub kind: AplotExpectedEventKind,
...
#[derive(Clone, Debug, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct AplotSourceRefinement {
...
#[derive(Clone, Debug, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct AplotDifferentialArtifact {
    pub schema_version: String,
    pub evaluator_version: String,
    pub source_digest_sha256: String,
    pub schedule_digest_sha256: String,
    pub sequence_digest_sha256: String,
    pub hardware_status: String,
    pub engine_digests: AplotEngineDigests,
    pub source_events: Vec<AplotExpectedEvent>,
    pub verdict: AplotDifferentialVerdict,
    pub source_reference: Option<EngineRunArtifact>,
    pub source_refinement: AplotSourceRefinement,
    pub oracle: DifferentialOracleArtifact,
}

impl AplotDifferentialArtifact {
    pub fn validate(&self) -> Result<(), AplotDifferentialError> {
        if self.schema_version != APLOT_DIFFERENTIAL_VERSION
            || self.evaluator_version != APLOT_DIFFERENTIAL_VERSION
            || self.hardware_status != HARDWARE_STATUS
            || !is_sha256(&self.source_digest_sha256)
            || !is_sha256(&self.schedule_digest_sha256)
            || !is_sha256(&self.sequence_digest_sha256)
            || !source_events_are_canonical(&self.source_events)
            || !engine_digests_match(&self.engine_digests, &self.oracle)
        {
            return Err(AplotDifferentialError::ArtifactContract);
        }
        self.oracle
            .validate()
            .map_err(|error| AplotDifferentialError::Oracle(error.to_string()))?;
    ...
    pub fn canonical_json(&self) -> Result<Vec<u8>, AplotDifferentialError> {
        self.validate()?;
        serde_json::to_vec(self)
            .map_err(|error| AplotDifferentialError::Serialization(error.to_string()))
    }

    pub fn artifact_sha256(&self) -> Result<String, AplotDifferentialError> {
        Ok(sha256_hex(&self.canonical_json()?))
    }
}

pub fn evaluate_aplot_differential(
    compiled: &AplotCompiledQsm,
    sequence: &AplotPublicSequence,
    engine_digests: &AplotEngineDigests,
) -> Result<AplotDifferentialArtifact, AplotDifferentialError> {
    evaluate_aplot_differential_with_host_tape(
        compiled,
        sequence,
        sequence.host_tape(),
        engine_digests,
...
pub fn evaluate_aplot_differential_with_host_tape(
    compiled: &AplotCompiledQsm,
    sequence: &AplotPublicSequence,
    ...
    let source_verdict = evaluate_aplot_source_reference(compiled, sequence)
        .map_err(|error| AplotDifferentialError::SourceReference(error.to_string()))?;
    let (source_reference, source_unresolved) = match source_verdict {
        AplotReferenceVerdict::Executed(artifact) => (
            Some(project_source_reference(artifact.run(), host_tape)?),
            None,
        ),
        AplotReferenceVerdict::Unresolved(reason) => (
            None,
    ...
    let artifact = AplotDifferentialArtifact {
        schema_version: APLOT_DIFFERENTIAL_VERSION.to_owned(),
        evaluator_version: APLOT_DIFFERENTIAL_VERSION.to_owned(),
        source_digest_sha256: hex(compiled.binding().source_digest.as_bytes()),
        schedule_digest_sha256: hex(compiled.binding().schedule_digest.as_bytes()),
        ...
        source_events: expected_source_events(compiled, sequence),
        verdict,
        source_reference,
        source_refinement,
        oracle,
...
fn expected_source_events(
    compiled: &AplotCompiledQsm,
    sequence: &AplotPublicSequence,
) -> Vec<AplotExpectedEvent> {
    let events = compiled
        .events()
        .iter()
    ...
    sequence
        ...
        .filter_map(|command| {
            ...
            let event = events.get(&(command.service_alias, command.public_slot))?;
            Some(AplotExpectedEvent {
                qsm_alias: event.qsm_alias,
                public_step: event.public_step,
                ...
                kind: match event.kind {
                    AplotPublicEventKind::FragmentAttempt => {
                        AplotExpectedEventKind::FragmentAttempt
                    }
                    AplotPublicEventKind::Reconnect => AplotExpectedEventKind::Reconnect,
                    AplotPublicEventKind::Deadline => AplotExpectedEventKind::Deadline,
                },
...
fn source_events_are_canonical(events: &[AplotExpectedEvent]) -> bool {
    events.iter().all(|event| {
        event.qsm_alias != 0
            && match event.kind {
                AplotExpectedEventKind::FragmentAttempt => {
                    event.fragment_ordinal.is_some()
                        && event.fragment_index.is_some()
                        && event.delivered.is_some()
                        && matches!(event.declared_fault_code, 0 | APLOT_PUBLIC_LOSS)
                }
                AplotExpectedEventKind::Reconnect => {
                    event.fragment_ordinal.is_none()
                        && event.fragment_index.is_none()
                        && event.delivered.is_none()
                        && event.declared_fault_code == APLOT_PUBLIC_RECONNECT
                }
                AplotExpectedEventKind::Deadline => {
                    event.fragment_ordinal.is_none()
                        && event.fragment_index.is_none()
                        && event.delivered.is_none()
                        && event.declared_fault_code == APLOT_PUBLIC_DEADLINE
                }
            }
    })
}
...
fn validate_host_tape_shape(
    ...
    {
        Err(AplotDifferentialError::HostTapeShape)
    } else {
        Ok(())
    }
}
...
fn project_source_reference(
    ...
    host_tape: &PublicHostTape,
) -> Result<EngineRunArtifact, AplotDifferentialError> {
    ...
    for event in &base.trace {
        ...
        let directive = host_tape
            .directives()
            .get(directive_index)
            .ok_or(AplotDifferentialError::HostTapeShape)?;
        if directive.import() != import {
            return Err(AplotDifferentialError::HostTapeShape);
        }
        directive_index = directive_index
            .checked_add(1)
            .ok_or(AplotDifferentialError::ArtifactContract)?;
...
fn small_step_tape(host_tape: &PublicHostTape) -> Result<PublicHostTape, AplotDifferentialError> {
    let directives = host_tape
        ...
        .map(|directive| {
            let import = directive
                .import()
                .strip_prefix("qseal.")
                .ok_or(AplotDifferentialError::HostImportContract)?;
            Ok(HostDirective::new(import, directive.outcome()))
        })
        .collect::<Result<Vec<_>, AplotDifferentialError>>()?;
...
fn append_lifecycle_event(
    ...
    match kind {
        CommandKind::PublicReset => match values {
            ...
            _ => return Err(AplotDifferentialError::ReturnShape("reset")),
        },
        CommandKind::PublicHandoff => match values {
            [Value::I64(value)] => trace.push(ObservableEvent::Handoff { value: *value }),
            _ => return Err(AplotDifferentialError::ReturnShape("handoff")),
        },
        CommandKind::PublicCall | CommandKind::PublicFault => {}
...
fn make_run(
    ...
    EngineRunArtifact::new(input, trace, termination, verdict)
        .map_err(|error| AplotDifferentialError::EngineContract(error.to_string()))
}
...
fn source_refinement(
...
) -> Result<AplotSourceRefinement, AplotDifferentialError> {
    let Some(source) = source else {
        return Ok(AplotSourceRefinement {
            verdict: AplotDifferentialVerdict::Unresolved,
    ...
    if !shared_input_matches(&source.input, &small_step.input) {
        return Err(AplotDifferentialError::SourceInputMismatch);
    }
    ...
    if let Some(first_difference) = first_difference(source, small_step) {
        ...
        Ok(AplotSourceRefinement {
            verdict: AplotDifferentialVerdict::Match,
            first_difference: None,
            unresolved_reason: None,
        })
    }
...
const fn aggregate_verdict(
    source: AplotDifferentialVerdict,
    oracle: DifferentialVerdict,
) -> AplotDifferentialVerdict {
    match (source, oracle) {
        (AplotDifferentialVerdict::Unresolved, _) | (_, DifferentialVerdict::Unresolved) => {
            AplotDifferentialVerdict::Unresolved
        }
        (AplotDifferentialVerdict::Counterexample, _)
        | (_, DifferentialVerdict::Counterexample) => AplotDifferentialVerdict::Counterexample,
        (AplotDifferentialVerdict::Match, DifferentialVerdict::Match) => {
            AplotDifferentialVerdict::Match
        }
    }
}

fn external_error(engine: &'static str, error: impl std::fmt::Display) -> AplotDifferentialError {
    AplotDifferentialError::ExternalEngine {
        engine,
...
pub enum AplotDifferentialError {
    ...
    #[error("APLOT differential artifact serialization failed: {0}")]
    Serialization(String),
    #[error("APLOT differential artifact violated its contract")]
    ArtifactContract,
