...
use quotient_limit_model::{
    ModelError, ModelLimits, ObserverId, PrivateHistoryId, PrivateHistoryModel,
};
...
pub enum TraceLpError {
    ...
    #[error("trace catalog is empty, oversized, or non-canonical")]
    InvalidTraceCatalog,
    #[error("trace observations do not exactly cover declared observers")]
    InvalidObservationProjection,
    #[error("history {history:?} has no utility-feasible trace")]
    NoUtilityFeasibleTrace { history: PrivateHistoryId },
    #[error("explicit LP exceeds a frozen resource limit")]
    ResourceLimit,
}
...
pub fn build_exact_trace_lp(
    ...
    let partition: BTreeMap<_, _> = model
        .action_quotient
        .class_of_history
        .iter()
        .map(|x| (x.history, x.class))
        .collect();
    let classes: BTreeMap<_, _> = model
        .action_quotient
        .classes
        .iter()
        .map(|x| (x.id, &x.semantics))
        .collect();
    let readiness: BTreeMap<_, _> = model
        .readiness
        .entries
        .iter()
        .map(|x| (x.history, x.ready_slot))
        .collect();
    ...
    for history in &model.histories {
        ...
        for trace in traces {
            ...
            if authorized && timely {
                let index =
                    u32::try_from(variables.len()).map_err(|_| TraceLpError::ResourceLimit)?;
                variables.push(TraceVariable {
                    history: *history,
                    trace: trace.id,
                });
                objective.push(LinearTerm {
                    variable: index,
                    coefficient: i64::try_from(trace.cost)
                        .map_err(|_| TraceLpError::ResourceLimit)?,
                });
            }
        }
    ...
    if variables.len() > limits.max_variables {
        return Err(TraceLpError::ResourceLimit);
    }

    let mut equalities = Vec::new();
    for history in &model.histories {
        equalities.push(Equality {
            kind: EqualityKind::Normalization { history: *history },
            terms: select_terms(&variables, |x| x.history == *history, 1),
            rhs: 1,
        });
    }
    for class in &model.action_quotient.classes {
        ...
        for right in members.into_iter().skip(1) {
            for observer in &model.observers.observers {
                let observations: BTreeSet<_> = traces
                    .iter()
                    .map(|trace| observation(trace, observer.id))
                    .collect();
                for observed in observations {
                    let mut terms = select_terms(
                        &variables,
                        |x| {
                            x.history == left
                                && observation(trace_by_id[&x.trace], observer.id) == observed
                        },
                        1,
                    );
    ...
    if equalities.len() > limits.max_constraints {
        return Err(TraceLpError::ResourceLimit);
    }
    Ok(ExplicitTraceLp {
        variables,
...
fn validate_traces(
    ...
    if traces.is_empty()
        || traces.len() > limits.max_traces
        || !traces.windows(2).all(|x| x[0].id < x[1].id)
    {
        return Err(TraceLpError::InvalidTraceCatalog);
    }
    ...
    if traces.iter().any(|trace| {
        ...
        return Err(TraceLpError::InvalidObservationProjection);
    }
    Ok(())
}
...
fn observation(trace: &TraceCandidate, observer: ObserverId) -> ObservationId {
    trace
        .observations
        .iter()
        .find(|x| x.observer == observer)
        .expect("validated observer projection")
        .observation
}

fn select_terms(
    variables: &[TraceVariable],
    include: impl Fn(&TraceVariable) -> bool,
    coefficient: i64,
) -> Vec<LinearTerm> {
    variables
        .iter()
        .enumerate()
        .filter(|(_, x)| include(x))
        .map(|(i, _)| LinearTerm {
            variable: i as u32,
            coefficient,
        })
...
impl ExplicitTraceLp {
    pub fn canonical_digest(&self) -> [u8; 32] {
        ...
        d.update(DOMAIN_MATRIX);
        d.update([0]);
        d.update((self.variables.len() as u64).to_le_bytes());
        for x in &self.variables {
            d.update(x.history.0.to_le_bytes());
            d.update(x.trace.0.to_le_bytes());
        }
        d.update((self.equalities.len() as u64).to_le_bytes());
        for row in &self.equalities {
            d.update((row.terms.len() as u64).to_le_bytes());
            for x in &row.terms {
                d.update(x.variable.to_le_bytes());
                d.update(x.coefficient.to_le_bytes());
            }
            d.update(row.rhs.to_le_bytes());
        }
...
mod tests {
            ...
            action_quotient: ActionQuotient {
                classes: vec![ActionQuotientClass {
                    ...
                    semantics: AuthorizedActionSemantics {
                        action_sequence: vec![ActionCode(1)],
                        service: ServiceId(0),
                        policy: PolicyId(0),
                        admission_cutoff: 3,
    ...
    fn traces() -> Vec<TraceCandidate> {
        vec![4, 6]
            .into_iter()
            .enumerate()
            .map(|(id, slot)| TraceCandidate {
                id: TraceId(id as u16),
                release_slot: slot,
                action_count: 1,
                action_code: ActionCode(1),
                service: ServiceId(0),
                observations: vec![ObserverObservation {
                    observer: ObserverId(0),
                    observation: ObservationId(slot),
                }],
                cost: slot.into(),
            })
            .collect()
    ...
    fn exact_rows_bind_each_observable_trace() {
        let lp = build_exact_trace_lp(
            &fixture(),
            &traces(),
            ModelLimits::default(),
            TraceLpLimits::default(),
        )
        .unwrap();
    ...
    fn incomplete_projection_is_rejected() {
        let mut traces = traces();
        traces[0].observations.clear();
        assert_eq!(
            build_exact_trace_lp(
                &fixture(),
                &traces,
                ModelLimits::default(),
                TraceLpLimits::default()
            ),
            Err(TraceLpError::InvalidObservationProjection)
        );
    }
}
