...
CORPUS = ROOT / "configs" / "quotient_forge" / "benchmark_cases" / "noticer"
REGISTRY = ROOT / "configs" / "quotient_forge" / "benchmark_family_registry_v1.yaml"

EXPECTED = {
    "noticer_aets_fixed_cadence": ("train", 2, 1, 2, 4),
    "noticer_aplot_bounded_loss": ("train", 3, 2, 2, 5),
    "noticer_atv2_action_window": ("train", 3, 1, 2, 2),
    "noticer_aepa_public_context": ("development", 3, 2, 2, 3),
    "noticer_service_separation": ("development", 4, 2, 3, 4),
    "noticer_reconnect_normalization": ("development", 4, 3, 2, 4),
    "noticer_multiservice_collusion": ("held_out", 4, 2, 4, 9),
    "noticer_longitudinal_handoff": ("held_out", 5, 2, 4, 7),
}
...
def test_noticer_case_source_binding_and_dimensions(family_id: str) -> None:
    ...
    source = (CORPUS / f"{family_id}.qf").read_bytes()
    verify_aqrs_source_binding(case, source)

    assert case.family_id == family_id
    assert case.variant_id == "canonical"
    assert case.split == split
    assert case.dimensions.horizon == horizon
...
def test_noticer_corpus_exactly_matches_locked_family_splits() -> None:
    registry = yaml.safe_load(REGISTRY.read_text(encoding="utf-8"))
    locked = {
        family["id"]: family["split"]
        for family in registry["families"]
        if family["category"] == "noticer"
    }
    actual = {
...
def test_required_attack_surfaces_and_obligations_are_covered() -> None:
    ...
    assert tags == {"collusion", "failure", "longitudinal", "retry", "silence", "size", "timing"}
    assert obligations == {"action_window", "bounded_loss", "exactly_once", "reconnect"}
